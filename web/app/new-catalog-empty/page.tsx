...
function PageInner() {
  ...
  /* В этом варианте фильтры друг друга не ограничивают, поэтому при смене
     срока выбранные чипы сохраняются как есть — даже если вместе со сроком
     они ничего не находят. Такой набор и показывает пустое состояние. */
  ...
  /* Отличие этого варианта от /new-catalog: чипы никогда не гаснут — любую
     комбинацию можно выбрать, а несовместимая просто приводит к пустому
     состоянию каталога ниже. Порядок прежний: выбранные уезжают вперёд. */
  const chips = FILTER_CHIPS
    .map(c => ({ ...c, on: activeChips.includes(c.key) }))
    .sort((a, b) => {
  ...
  /* Ни один продукт не прошёл фильтры — вместо секций показываем пустое
     состояние. Достижимо только здесь: в /new-catalog такую комбинацию не
     дают выбрать (чип гаснет). */
  const isEmpty =
    displayBest.length === 0 &&
    visibleAvailableNow.length === 0 &&
  ...
  return (
    <div className="screen page-enter nc-catalog" id="top">
                ...
                <div className="cat-filters-block">
                  ...
                  <div className="cat-chips-scroll" ref={chipsRowRef}>
                    {chips.map(chip => (
                      <button
                        key={chip.key}
                        className={`cat-chip${chip.on ? " active" : ""}`}
      ...
      <div className="lower-panel cat-lower">
        ...
        <div style={{ visibility: showSkeleton ? "hidden" : "visible", width: "100%" }}>
          {isEmpty ? (
            <div className="cat-empty">
              <img src={asset("/images/analytics/empty-payouts.png")} alt="" className="cat-empty-img" />
              <div className="cat-empty-text">
                <p className="cat-empty-title">Ничего не нашлось</p>
                <p className="cat-empty-subtitle">
                  Попробуйте снять один из фильтров или выбрать другой срок
                </p>
              </div>
              <button className="cat-empty-btn" onClick={resetFilters}>
                <span className="btn-up-label">Сбросить фильтры</span>
              </button>
            </div>
          ) : (
            <>
        ...
        {showSkeleton && !isEmpty && <SkeletonGrid sections={[visibleBest, visibleAvailableNow, visibleAccounts, visibleDeposits, visibleAlt]} />}
      </div>
