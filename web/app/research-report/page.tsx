...
export default function ResearchReportPage() {
  ...
  const heatmapPids = useMemo(() => [...new Set(scopedSessions.map((s) => s.pid))], [scopedSessions]);
  const heatmapPaths = useMemo(() => [...new Set((events ?? []).filter((e): e is ClickEvent => e.type === "click" && e.scenario === "showcase_test" && (heatmapPid === "all" || e.pid === heatmapPid)).filter((click) => {
  ...
  }).map((click) => click.path))], [events, heatmapPid, heatmapScope, scopedSessions]);
  const selectedHeatmapPath = heatmapPath && heatmapPaths.includes(heatmapPath) ? heatmapPath : heatmapPaths[0] ?? null;
  const heatmapClicks = useMemo(() => (events ?? []).filter((e): e is ClickEvent => e.type === "click" && e.scenario === "showcase_test" && e.path === selectedHeatmapPath && (heatmapPid === "all" || e.pid === heatmapPid)), [events, heatmapPid, selectedHeatmapPath])
    .filter((click) => {
      if (heatmapScope === "all") return true;
  ...
  return <main style={styles.page}>
    ...
    {events === null ? <p style={styles.muted}>Загружаем данные…</p> : <>
      ...
      <section style={styles.card}><div style={styles.sectionTop}><div><h2 style={styles.heading}>Тепловые карты</h2><p style={styles.description}>Все срезы доступны здесь: общий, по сценарию и по респонденту.</p></div></div>
        <div style={styles.heatmapControls}><div style={styles.controlGroup}><span style={styles.controlLabel}>Сценарий</span>{[{ id: "all" as const, label: "Общий" }, ...prototypes.map((p) => ({ id: p.id, label: p.label }))].map((item) => <button key={item.id} onClick={() => { setHeatmapScope(item.id); setHeatmapPid("all"); setHeatmapPath(null); }} style={{ ...styles.smallFilter, ...(heatmapScope === item.id ? styles.smallFilterActive : {}) }}>{item.label}</button>)}</div><div style={styles.controlGroup}><span style={styles.controlLabel}>Респондент</span><button onClick={() => { setHeatmapPid("all"); setHeatmapPath(null); }} style={{ ...styles.smallFilter, ...(heatmapPid === "all" ? styles.smallFilterActive : {}) }}>Все</button>{heatmapPids.map((pid) => <button key={pid} onClick={() => { setHeatmapPid(pid); setHeatmapPath(null); }} style={{ ...styles.smallFilter, ...(heatmapPid === pid ? styles.smallFilterActive : {}) }}>{participantName(pid, pids)}</button>)}</div><div style={styles.controlGroup}><span style={styles.controlLabel}>Экран</span>{heatmapPaths.map((path) => <button key={path} onClick={() => setHeatmapPath(path)} style={{ ...styles.smallFilter, ...(selectedHeatmapPath === path ? styles.smallFilterActive : {}) }}>{labelPath(path)}</button>)}</div></div>
        <Heatmap clicks={heatmapClicks} path={selectedHeatmapPath} title={`${heatmapScope === "all" ? "Общая карта" : prototypes.find((p) => p.id === heatmapScope)?.label}${heatmapPid === "all" ? "" : ` · ${participantName(heatmapPid, pids)}`}${selectedHeatmapPath ? ` · ${labelPath(selectedHeatmapPath)}` : ""}`} />
      </section>
