...
// В отчёте показываем только тот экран, который исследуется в этом задании:
// каталог накоплений с фильтрами. Остальные посещённые экраны не должны
// становиться отдельными вариантами тепловой карты.
const HEATMAP_PATH = "/new-catalog";
 ...
 * success event, so include it explicitly in the report's terminal path. */
function pathUntilSuccess(paths: string[], prototype: ShowcasePrototype, succeeded: boolean) {
  if (!succeeded) return paths;
  const target = targetProductPath(prototype);
  return paths.some((path) => path.split("?")[0] === target.split("?")[0]) ? paths : [...paths, target];
}
...
function sessionsFrom(events: AnalyticsEvent[]): Session[] {
  ...
  return starts.map((start) => {
    ...
    return {
      ...
      path: [...new Set(pathUntilSuccess(screenEvents.map((e) => e.path), start.prototype, !!success).map(labelPath))],
    };
  });
}
...
function journeysFrom(events: AnalyticsEvent[], prototype: ShowcasePrototype | "all", includedSessionIds: Set<string>): JourneyPath[] {
  ...
  return starts.map((start) => {
    ...
    const visited = events.filter((e): e is ScreenTimeEvent => e.type === "screen_time" && e.pid === start.pid && e.scenario === "showcase_test" && e.timestamp >= start.timestamp && (!end || e.timestamp < end) && (!success || e.timestamp <= success.timestamp))
      .sort((a, b) => a.timestamp - b.timestamp)
      .map((e) => e.path);
    const nodes = ["/showcase-test", ...pathUntilSuccess(visited, start.prototype, !!success)].filter((path, i, all) => i === 0 || path !== all[i - 1]);
